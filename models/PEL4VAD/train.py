...
def train_func(dataloader, model, optimizer, criterion, criterion2, lamda=0):
    ...
    with torch.set_grad_enabled(True):
        ...
        for i, (v_input, t_input, label, multi_label) in enumerate(dataloader):
            ...
            t_input = t_input.float().to(DEVICE, non_blocking=True)
            label = label.float().to(DEVICE, non_blocking=True)
            multi_label = multi_label.to(DEVICE, non_blocking=True)
            ...
            ground_truth = torch.tensor(gen_label(video_labels), dtype=v_feat.dtype).to(DEVICE)
